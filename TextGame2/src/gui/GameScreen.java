...
public class GameScreen extends JPanel implements ActionListener {
		...
		for (int i = 0; i < playerMap.getWidth(); i++) {
			for (int j = 0; j < playerMap.getHeight(); j++) {
				if (playerMap.get(i, j) != null) {
					...
					g2d.drawImage(unit.getBG(), i * 50, j * 50, this);
					if (unit.hasCharacter())
						g2d.drawImage(unit.getCharacter().getCharacterImage(),
								i * 50, j * 50, this);
				}
			}
		}
		g2d.drawImage(player.getCharacterImage(), player.getXPos() * 50,
	...
	private void paintMapPlayerFixed(GameMap playerMap, Graphics2D g2d) {
		for (int i = -2; i < 3; i++) {
			for (int j = -2; j < 3; j++) {
				if (playerMap.get(player.getXPos() + i, player.getYPos() + j) != null) {
					...
					g2d.drawImage(unit.getBG(), (i + 3) * 50, (j + 3) * 50, this);
					if (unit.hasCharacter())
						g2d.drawImage(unit.getCharacter().getCharacterImage(),
								(i + 3) * 50, (j + 3) * 50, this);
				}
		...
		g2d.drawImage(player.getCharacterImage(), 3 * 50, 3 * 50, this);
	}
